...
void main()
{
	...
	if (isnan(depthvalue)) {
		Filteroutput = colorval;
		return;	//safety check if depth is not a number
	}
	float cum_w = 0.0;
	for (int i = 0; i < 9; i++) 
	{
		ivec2 offset = ivec2(-1 + i % 3, -1 + i / 3);
		ivec2 uv = Texel + offset * stepwidth;
		...
		float c_w = max(min(1.0 - dot(ct, ct) / c_phi * sf2, 1.0), 0.0);
		float pt = abs(depthvalue - depthtemp);
		float p_w = max(min(1.0 - pt/p_phi, 1.0), 0.0);
		float weight = c_w * p_w * n_w * kernel[i%3][i/3];
