...
namespace rtf
{
	...
	void Renderpass_Gui::buildCommandBuffer()
	{
		...
		renderPassBeginInfo.renderPass = m_Main->renderPass;
		...
		for (int32_t i = 0; i < m_Main->drawCmdBuffers.size(); ++i)
		{
			renderPassBeginInfo.framebuffer = m_Main->frameBuffers[i];
	...
	void Renderpass_Gui::PreparePipelines()
	{
		...
		shaderStages[0] = m_Main->LoadShader("gui/gui.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = m_Main->LoadShader("gui/gui.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
