...
//	}
//
//	void RenderpassPathTracer::updateUniformBuffers(float timer, Camera* camera)
//	{
//		m_uniformData.projInverse = glm::inverse(camera->matrices.perspective);
//		m_uniformData.viewInverse = glm::inverse(camera->matrices.view);
//		m_uniformData.lightPos = glm::vec4(cos(glm::radians(timer * 360.0f)) * 40.0f, -50.0f + sin(glm::radians(timer * 360.0f)) * 20.0f, 25.0f + sin(glm::radians(timer * 360.0f)) * 5.0f, 0.0f);
...
//	}
//
//	void RenderpassPathTracer::buildCommandBuffer(VkCommandBuffer commandBuffer, VkImage swapchainImage, uint32_t width, uint32_t height)
//	{
//		/*
...
//		vkCmdPushConstants(commandBuffer, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstant), &m_pushConstant);
//
...
//		vkCmdCopyImage(commandBuffer, m_storageImage.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, swapchainImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);
//
...
//	}
//
//	VkPhysicalDeviceAccelerationStructureFeaturesKHR* RenderpassPathTracer::getEnabledFeatures()
//	{
...
//		return vkGetBufferDeviceAddressKHR(m_vulkanDevice->logicalDevice, &bufferDeviceAI);
//	}
//
...
//	}
//	
//	void RenderpassPathTracer::enableExtensions(std::vector<const char*>& enabledDeviceExtensions)
//	{
